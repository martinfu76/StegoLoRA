...
uses Llama 3 8B for routing and GPT-2 for watermark generation/extraction.
...
| `corpus_build.py` | Build verified watermarked training carriers |
| `hash_watermark.py`, `embed.py` | Framed payload embedding and extraction |
| `agent.py`, `mcp_server.py` | Tool-call controller and tokenizer-only MCP receiver |
...
```powershell
python pipeline.py send --model llama3-8b --model-dir D:\models --watermark-model gpt2 --watermark-model-dir D:\models --message HELLO --key MY-SECRET --bits-per-token 2 --max-new-tokens 96 --output .\out.txt
```

Load Llama and the adapter locally, then invoke MCP:

```powershell
python pipeline.py receive --model llama3-8b --model-dir D:\models --watermark-model gpt2 --watermark-model-dir D:\models --adapter-path .\adapters\stegolora --input .\out.txt --key MY-SECRET --bits-per-token 2 --prompt-format auto --verbose
```
...
## Ubuntu L40 vLLM Deployment

The supported server target is Ubuntu with an NVIDIA L40-class GPU. Copy the
...
```powershell
python experiments.py run --profile practical --model llama3-8b --model-dir D:\models --watermark-model gpt2 --watermark-model-dir D:\models --corpus-path .\experiment_corpus.json --output-root .\experiments\practical --device cuda:0 --dtype float16 --qlora --num-gpus 1 --batch-size 2 --gradient-accumulation-steps 8 --max-length 256 --eval-offset 300 --eval-samples 50 --normal-samples 50 --load-in-4bit-eval --mcp-samples 0 --resume
```
